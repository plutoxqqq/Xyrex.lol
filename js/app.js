...
const NO_OFFICIAL_DISCORD_MESSAGE = (window.XyrexData && window.XyrexData.NO_OFFICIAL_DISCORD_MESSAGE) || 'This script does not have an official discord server';
...
function getPriceControls() {
  if (!filtersModule && window.XyrexModules?.createFiltersModule) filtersModule = window.XyrexModules.createFiltersModule({ qs, qsa, products, renderProducts });
  return filtersModule ? filtersModule.getPriceControls() : { free: false, paid: false, both: false };
}
...
function applyAllFilters() {
  if (!filtersModule && window.XyrexModules?.createFiltersModule) filtersModule = window.XyrexModules.createFiltersModule({ qs, qsa, products, renderProducts });
  if (!filtersModule) return;
  filtersModule.applyAllFilters();
}
...
function initScriptsHub() {
  ...
  if (!tabsModule && window.XyrexModules?.createTabsModule) {
    tabsModule = window.XyrexModules.createTabsModule({ qsa, setActiveSubtab, syncSubtabButtons, syncNavButtonsWithPage, setActivePage });
  }
  tabsModule?.bindTabEvents();
...
function initCollapsibleFilters() {
  if (!filtersModule && window.XyrexModules?.createFiltersModule) {
    filtersModule = window.XyrexModules.createFiltersModule({ qs, qsa, products, renderProducts });
  }
  filtersModule?.initCollapsibleFilters();
}

function hideInitialLoadingOverlay() {
  if (window.XyrexModules?.hideInitialLoadingOverlay) {
    window.XyrexModules.hideInitialLoadingOverlay();
    return;
  }
